# Prompt Qualifier — the Evaluator used to qualify Inquire's prompt

This is the Session-5 *Prompt Evaluation Assistant* — the artefact at the heart of the assignment.

You hand it any candidate system prompt; it returns a **JSON scorecard** across 9 criteria. You iterate on your draft until every boolean comes back `true`. The final iteration is what ships in `talk2mcp.py`.

## How to use it

**Option A — ChatGPT / Claude / Cursor (manual):**
Copy everything between the `<!--EVALUATOR_START-->` and `<!--EVALUATOR_END-->` markers below into ChatGPT. After the last line, paste your candidate prompt. The chat returns the scorecard.

**Option B — terminal (automated):**
```bash
uv run python evaluate_prompt.py v1    # score the draft
uv run python evaluate_prompt.py v2    # score the production prompt
```
`evaluate_prompt.py` reads the evaluator block from this file, picks V1 (a constant) or V2 (the prompt in `talk2mcp.py`), and sends both to the configured LLM provider. Same scorecard, no copy-paste.
